...
# Decode carries a small linear x-term through dim1:
# z_eff = h2 + h1/eps ≈ (s - 10*w) + CARRY_SLOPE * x
CARRY_SLOPE = -0.1
...
def hand_set_weights_better(model: Model) -> None:
    ...
    params["model"]["norm"]["weight"] = mx.array(
        [1.0 / CONST_NORM, CARRY_SLOPE * DIGIT_SCALE * DECODE_LINEAR_EPS, DIGIT_SCALE],
        dtype=mx.float32,
    )
    ...
    # - exact wrap pair for w = I[s + c >= 10] via m = -2*x + 20*s - 189
    # - z-predecode channel stores (s - 10*w)
    # - carry surrogate (-0.1*x) is injected at decode through final norm dim1.
    gate = np.zeros((INTERMEDIATE_SIZE, MODEL_DIM), dtype=np.float32)
    # wrap pair: relu(m + 1) - relu(m), m = -2*x + 20*s - 189
