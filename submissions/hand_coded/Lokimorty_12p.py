"""
AdderBoard submission: 12-parameter qwen-derived adder.

Interface expected by verify.py:
...
class MLP(nn.Module):
    class StructuredGateProj(nn.Module):
        def __init__(self):
            super().__init__()
            self.weight = mx.zeros((3,), dtype=mx.float32)
    ...
    class StructuredDownProj(nn.Module):
        def __init__(self):
            super().__init__()
            self.weight = mx.zeros((1,), dtype=mx.float32)
        ...
        def __call__(self, x: mx.array) -> mx.array:
            t = self.weight[0]
            out0 = mx.zeros_like(x[..., 0])
            out1 = t * (x[..., 1] - x[..., 0])
            return mx.stack([out0, out1], axis=-1)

    class SharedUpProj(nn.Module):
        ...
        def __call__(self, x: mx.array) -> mx.array:
            s = x[..., 0] * self.weight[0]
            return mx.broadcast_to(s[..., None], s.shape + (self.hidden_dim,))

    def __init__(self, hidden_dim: int):
        super().__init__()
        self.gate_proj = MLP.StructuredGateProj()
        self.down_proj = MLP.StructuredDownProj()
        self.up_proj = MLP.SharedUpProj(hidden_dim)
...
def _build_model_args() -> ModelArgs:
    return ModelArgs(
        model_type="qwen3-parametric-embedding",
...
def _init_weights(model: Model) -> None:
    ...
    attn["v_proj"]["weight"] = mx.array([-22.0 * DIGIT_SCALE], dtype=mx.float32)
    attn["q_proj"]["weight"] = mx.array([math.cos(PHI), -math.sin(PHI)], dtype=mx.float32)

    gate = np.zeros((3,), dtype=np.float32)
    gate[0] = CARRY_ALPHA * (-94.0) / CONST_NORM
    gate[1] = CARRY_ALPHA * (-95.0) / CONST_NORM
    gate[2] = CARRY_ALPHA * DIGIT_SCALE
    layer["mlp"]["gate_proj"]["weight"] = mx.array(gate, dtype=mx.float32)

    layer["mlp"]["up_proj"]["weight"] = mx.array([1.0 / CONST_NORM], dtype=mx.float32)
    layer["mlp"]["down_proj"]["weight"] = mx.array([100.0 / CARRY_ALPHA], dtype=mx.float32)
    model.update(params)
...
def build_model():
    ...
    _init_weights(model)
    metadata = {
        "name": "qwen12_parametric_embedding",
        "author": "Lokimorty",
        "params": _count_parameters(model.parameters()),
        "architecture": "qwen-derived 1L h=2 with structured attn/mlp + parametric tied embedding",
        "tricks": [
            "RoPE period-19 geometry",
            "shared/structured projections",
            "2-parameter embedding e(d)=[c0-c1*d^2,-d]",
            "constructive carry hinge",
        ],
    }
